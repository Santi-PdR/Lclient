...
public final class LClientHud {
    ...
    public static void notify(String text) {
        NOTICES.addFirst(new Notice(text, System.currentTimeMillis()));
    ...
    public static void recordCombat(String text) {
        ...
        while (COMBAT.size() > 5) COMBAT.removeLast();
    }
    ...
    public static void onSound(PlaySoundEvent event) {
        ...
        if (sound.getSource() == SoundSource.MUSIC || sound.getSource() == SoundSource.AMBIENT || sound.getSource() == SoundSource.WEATHER) return;
        ...
        SOUNDS.addFirst(new SoundPing(sound.getX(), sound.getZ(), kind, System.currentTimeMillis()));
        while (SOUNDS.size() > 12) SOUNDS.removeLast();
    }

    private static String classify(String path) {
        if (containsAny(path, "gun", "shoot", "shot", "firearm", "rifle", "pistol", "revolver", "sniper")) return "DISPARO";
        if (containsAny(path, "explode", "explosion", "grenade", "blast")) return "EXPLOSIÓN";
        if (containsAny(path, "step", "footstep")) return "PASOS";
        if (containsAny(path, "door", "trapdoor", "gate", "chest", "barrel")) return "INTERACCIÓN";
        if (containsAny(path, "break", "place", "dig")) return "BLOQUE";
    ...
    private static void renderTargetAndCombat(GuiGraphics graphics, Minecraft minecraft, long now) {
        ...
        if (minecraft.hitResult instanceof EntityHitResult hit) {
            ...
            if (entity instanceof LivingEntity living) data += " | " + Math.round(living.getHealth()) + "/" + Math.round(living.getMaxHealth()) + " HP";
            int width = Math.max(minecraft.font.width(title), minecraft.font.width(data)) + 12;
            graphics.fill(x, y, x + width, y + 31, 0xB010141B);
            graphics.drawString(minecraft.font, title, x + 6, y + 5, 0xFFE8F4FF, false);
            graphics.drawString(minecraft.font, data, x + 6, y + 18, 0xFFA8B7C6, false);
            y += 37;
        }
        ...
        for (CombatEntry entry : COMBAT) {
            if (count++ >= 3) break;
            int alpha = (int) (255 * Mth.clamp(1.0F - (now - entry.time) / 12000.0F, 0.15F, 1.0F));
            graphics.drawString(minecraft.font, entry.text, x, y, (alpha << 24) | 0xFFB4B4, true);
            y += 11;
        }
    }
    ...
    private static void renderNotices(GuiGraphics graphics, Minecraft minecraft, long now) {
        ...
        for (Notice notice : NOTICES) {
            if (shown++ >= 4) break;
            int width = minecraft.font.width(notice.text) + 12;
            int x = minecraft.getWindow().getGuiScaledWidth() - width - 8;
            graphics.fill(x, y, x + width, y + 20, 0xC0121720);
            graphics.drawString(minecraft.font, notice.text, x + 6, y + 6, 0xFFE8F4FF, false);
            y += 24;
        }
    }
    ...
    private static void renderSoundRadar(GuiGraphics graphics, Minecraft minecraft, long now) {
        ...
        for (SoundPing ping : SOUNDS) {
            ...
            String arrow = arrow(relative);
            String text = arrow + " " + ping.kind + " " + distance + "m";
            int width = minecraft.font.width(text) + 10;
            graphics.fill(centerX - width / 2, y - 3, centerX + width / 2, y + 9, 0x8E0B0F14);
            graphics.drawCenteredString(minecraft.font, text, centerX, y, 0xFFF4E6C1);
            y += 14;
        }
    }
    ...
    private record SoundPing(double x, double z, String kind, long time) {}
